...
공식의 단일 원본은 `custom_components/lotto_645/lotto_core/`입니다. HA는 이 코어로 번호를 생성하고 **추첨 전 실제 저장한 추천·구매번호만** 결과와 대조하여 리뷰합니다. Lotto Lab은 같은 버전의 코어를 내려받아 별도로 평가하며, HA에는 과거 재실행·모의 점수 집계·다중 게임 설계가 없습니다.

기본 선택은 부분 Fisher–Yates 한 개입니다. 기존 선택·센서 ID·실제 추천·구매·리뷰 기록을 보존합니다. 조건 지정 균등 생성은 **구성 → 조건 지정 생성 설정**에서 조건을 저장한 뒤 추천 설정에서 선택합니다. 기존 다른 공식에 이 조건을 강제 적용하지 않습니다.

로또 복권 **한눈에**는 이번 회차의 최신 추천번호와 이전 회차의 당첨 대조 결과를 구분해 표시합니다. 공식 추가·제거, 번호 재생성, 통합 다시 불러오기 후 WebSocket 알림으로 갱신합니다. 작성 중인 구매번호는 덮어쓰지 않습니다.

[코어 API·Lotto Lab 연결 계약](docs/CORE_API.md) · [공식 사용법](docs/RECOMMENDED_FORMULAS.md)
...
19. [조건 공식 · 조건 지정 균등 생성](docs/methods/constraint_uniform.md)
20. [분할 위험 참고 · 비인기 패턴 회피](docs/methods/crowd_pattern_avoidance.md)
21. [합의 추천 · 선택 공식 중앙값](docs/methods/selected_median_consensus.md)
22. [합의 추천 · 계열별 다수결](docs/methods/selected_vote_consensus.md)

Home Assistant AI는 22개 로컬 공식과 별도이며 생성된 번호를 설명합니다.
...
번호는 백엔드 CCSS 공식과 OS CSPRNG가 확정하고, 설정 → AI의 기본 데이터 생성 AI 또는 선택한 `ai_task.*` 엔티티는 근거만 설명합니다. AI가 번호를 반환하거나 추첨 공식을 바꾸면 거절합니다. AI는 위 22개 로컬 공식이나 중앙값 집계에 포함되지 않습니다.
