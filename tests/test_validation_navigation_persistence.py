...
def test_historical_validation_survives_panel_navigation_contract():
    ...
    assert "hass.async_create_task(" in backend
    assert "await asyncio.shield(task)" in backend
    assert "lotto_645/historical_validation_state" in backend
    assert "'validation_states'" in backend

    assert "request('historical_validation_state')" in frontend
    assert "state.status === 'running'" in frontend
    assert "state.status === 'completed'" in frontend
    assert "this.render(state.result)" in frontend
    assert "다른 페이지로 이동해도 계산은 계속됩니다" in frontend
