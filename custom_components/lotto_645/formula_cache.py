...
from .methods import METHODS_BY_ID, formula_settings, resolve_method_definition
...
def cache_key(history, method_ids, nonce, options=None):
    ...
    if options is not None:
        key["generator_settings"] = formula_settings(options)
    return key
...
def restore_tickets(cache, history, method_ids, nonce, options=None):
    ...
    try:
        tickets = {
            key: validate_sampled_ticket(resolve_method_definition(key, options).sampling, values)
