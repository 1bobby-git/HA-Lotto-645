...
KEY = DOMAIN + '_panel'
PATH = 'lotto-645'
PANEL_TAG = 'lotto-ticket-panel-v2-2-0'
...
def _publish_panel(hass: HomeAssistant, shared: dict) -> None:
    ...
    if existing is not None:
        ...
        if config.get('_panel_custom', {}).get('name') not in {'lotto-ticket-panel','lotto-ticket-panel-v2-0-0','lotto-ticket-panel-v2-0-1','lotto-ticket-panel-v2-0-2','lotto-ticket-panel-v2-0-3','lotto-ticket-panel-v2-1-0',PANEL_TAG}:
...
def async_remove_ticket_panel(hass: HomeAssistant, entry_id: str, *, permanent: bool = False) -> None:
    ...
    if shared['entries']:
        ...
        existing = hass.data.get(frontend.DATA_PANELS, {}).get(PATH)
        if existing and (getattr(existing, 'config', None) or {}).get('_panel_custom', {}).get('name') in {'lotto-ticket-panel','lotto-ticket-panel-v2-0-0','lotto-ticket-panel-v2-0-1','lotto-ticket-panel-v2-0-2','lotto-ticket-panel-v2-0-3','lotto-ticket-panel-v2-1-0',PANEL_TAG}:
