...
import {reviewPresentation} from './lotto-panel-view.js?v=2.2.4';
...
if (!customElements.get('lotto-panel-tools-v2-2-4')) customElements.define('lotto-panel-tools-v2-2-4', LottoPanelTools);

export function applyPanelTools(panel) {
  const root = panel.shadowRoot, main = root?.querySelector('main'); if (!main) return;
  if (!root.querySelector('style[data-lotto-panel-tools-v2-2-4]')) {
    const style = element('style'); style.dataset.lottoPanelTools = VERSION; style.textContent = PANEL_STYLE; root.append(style);
  }
  if (!root.querySelector('lotto-panel-tools-v2-2-4')) {
    const tools = document.createElement('lotto-panel-tools-v2-2-4'); tools.panel = panel; main.prepend(tools);
  ...
  if (!panel._toolsDataHook) {
    ...
    panel.updateResults = function (data, ...args) {
      ...
      this.shadowRoot.querySelector('lotto-panel-tools-v2-2-4')?.setData(data); return result;
    };
  }
}
