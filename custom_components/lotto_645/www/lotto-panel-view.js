...
.shell{min-height:100%;padding-bottom:20px} .wrap{max-width:1248px;margin:0 auto;padding:0 40px}
...
/* A single full-width draw stage replaces the previous stacked card layout. */
.draw-stage{overflow:hidden;position:relative;border-radius:24px;background:radial-gradient(ellipse at 95% -20%,#314774 0,transparent 63%),#182237;color:#fff;padding:32px 36px 0;box-shadow:0 16px 36px #16244012}
.draw-main{display:grid;grid-template-columns:205px minmax(0,1fr);gap:30px;align-items:center;min-height:135px}
.draw-kicker{color:#b9c5d9;font-size:12px;margin-bottom:9px}.draw-stage h2{font-size:32px;font-weight:750;letter-spacing:-.035em}
...
.draw-stage :focus-visible{outline-color:#a9c7ff}.draw-details{background:#0c142530;margin:0 -36px;padding:0 36px;font-size:12px;color:#b9c5d9}
.draw-details summary{min-height:40px;align-content:center;cursor:pointer;list-style-position:inside}.draw-details p{padding:0 0 16px;max-width:870px;line-height:1.9}.draw-details a{color:#bfd6ff;margin-right:14px;display:inline-block;min-height:32px}
/* Wallet: paper-like number rows, with registration moved into an on-demand sheet. */
.dashboard-grid{display:grid;grid-template-columns:minmax(0,1fr) 304px;gap:32px;margin-top:36px;align-items:start}
...
.footer .sync{white-space:nowrap}.wallet-toolbar{display:flex;align-items:center;justify-content:space-between;gap:14px;margin-bottom:18px}.wallet-toolbar .select-wrap{display:flex;align-items:center;gap:12px}.wallet-toolbar label{font-size:13px;color:var(--muted);white-space:nowrap;margin:0}.wallet-toolbar select{width:190px}
...
dialog{inset:0 0 0 auto;margin:0;width:min(580px,100%);max-width:none;height:100dvh;max-height:none;border:0;border-left:1px solid var(--line);padding:0;color:var(--ink);background:var(--surface);box-shadow:-12px 0 60px #0a17382b;overflow:hidden;outline:0}
...
.sheet-foot{padding:17px 26px max(20px,env(safe-area-inset-bottom));border-top:1px solid var(--line);background:var(--surface);flex:none}.draft-status{font-size:12px;margin-bottom:11px;color:var(--muted);text-align:center}.sheet-actions{display:flex;gap:10px}.sheet-actions .primary{flex:1;min-height:50px}.sheet-actions .ghost{font-size:13px}.sheet-privacy{display:flex;align-items:center;justify-content:center;gap:6px;font-size:11px;color:var(--muted);padding:5px 0}.sheet-privacy svg{width:14px;height:14px}
.notice{position:fixed;z-index:12;left:50%;bottom:22px;transform:translateX(-50%);max-width:min(650px,calc(100vw - 32px));width:max-content;background:var(--ink);color:var(--surface);border-radius:14px;padding:14px 20px;box-shadow:0 6px 26px #15213626;font-size:13px;line-height:1.8;white-space:pre-line;overflow-wrap:anywhere}.notice:empty{display:none}.notice[data-error="true"]{background:var(--danger-soft);color:var(--danger);border:1px solid var(--danger)}
...
<div class="shell">
  ...
  <main class="wrap">
    ...
    <section id="screen-review" class="screen" role="tabpanel" aria-labelledby="tab-review" tabindex="0" hidden>
      <div class="page-heading"><div><div class="kicker">RECOMMENDATION REVIEW</div><h1>추천을 돌아보는 시간.</h1><p>추천 방식의 실제 결과를 차곡차곡 비교해 보세요.</p></div><span id="method-count" class="review-count"></span></div>
      <div class="review-grid"><section class="review-block" aria-labelledby="predictions-heading"><div class="section-heading"><h2 id="predictions-heading">이번 추첨, 추천번호 결과</h2></div><p class="review-note">추첨 전에 저장된 추천과 발표된 당첨번호를 비교합니다.</p><div class="table-scroll"><table class="mobile-table" role="table"><caption class="sr-only">추천 방식별 번호와 이번 추첨 판정</caption><thead role="rowgroup"><tr role="row"><th scope="col">추천 방식</th><th scope="col">번호</th><th scope="col">결과</th></tr></thead><tbody id="predictions" role="rowgroup"></tbody></table></div></section>
      <section class="review-block" aria-labelledby="reviews-heading"><div class="section-heading"><h2 id="reviews-heading">방식별 누적 리뷰</h2></div><p id="reviewstatus" class="review-note"></p><div class="table-scroll"><table class="mobile-table" role="table"><caption class="sr-only">추천 방식별 누적 별점, 평가 회차, 이번 점수, 정확 일치와 인접 번호, 순위</caption><thead role="rowgroup"><tr role="row"><th scope="col">추천 방식 / 누적 별점</th><th scope="col">평가 회차</th><th scope="col">이번 점수</th><th scope="col">정확 / ±1</th><th scope="col">순위</th></tr></thead><tbody id="reviews" role="rowgroup"></tbody></table></div></section>
