...
def build_analysis(
    ...
    *, rng=None, progress_callback=None, recommendation_callback=None,
) -> AnalysisResult:
    ...
    emitted = set()
    def emit(row):
        if recommendation_callback is not None and row.method_id not in emitted:
            emitted.add(row.method_id)
            recommendation_callback(deepcopy(row.to_storage()))
    ...
    for index, method_id in enumerate(execution_method_ids, start=1):
        ...
        if method.sampling:
            ...
            if method_id == "crowd_pattern_avoidance":
                from .preference import recommendation_details
                details.update(recommendation_details(combo))
                details["eligible_combination_count"] = None
    ...
    summary = {
        "algorithm": "lotto_core_1",
        "core_version": "1.21.0", "api_version": 1,
        "generation_sequence": generation_nonce,
    ...
    result = refresh_consensus(result, selected_method_ids, history)
