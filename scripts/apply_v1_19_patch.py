...
replace_once("custom_components/lotto_645/const.py", 'VERSION = "1.18.1"', 'VERSION = "1.19.0"')
replace_once("custom_components/lotto_645/manifest.json", '"version": "1.18.1"', '"version": "1.19.0"')
for path in (
    "custom_components/lotto_645/www/lotto-panel-shell.js",
    "custom_components/lotto_645/www/lotto-panel-validation.js",
):
    p = Path(path)
    text = p.read_text()
    if "1.18.1" not in text:
        raise SystemExit(f"{path}: expected v1.18.1 marker")
    p.write_text(text.replace("1.18.1", "1.19.0"))

Path("custom_components/lotto_645/korean_birthplaces.py").write_text(
...
if insertion not in text:
    if text.count(marker) != 1:
        raise SystemExit("config_flow import marker mismatch")
...
'''
new = '''    birth_place = str(options.get(CONF_SAJU_BIRTH_PLACE, "") or "").strip()
    birthplace_marker = (
...
'''
if text.count(old) != 1:
    raise SystemExit("config_flow timezone schema marker mismatch")
text = text.replace(old, new, 1)
old = '''            _required_text_marker(CONF_SAJU_BIRTH_PLACE, options): selector.TextSelector(),
            vol.Required(CONF_SAJU_TIMEZONE, default=timezone): selector.TextSelector(),
'''
new = '''            birthplace_marker: selector.SelectSelector(
...
'''
if text.count(old) != 1:
    raise SystemExit("config_flow birthplace field marker mismatch")
text = text.replace(old, new, 1)
old = '''            pending[CONF_SAJU_BIRTH_PLACE] = str(
...
'''
if text.count(old) != 1:
    raise SystemExit("config_flow saju submit marker mismatch")
text = text.replace(old, new, 1)
old = '''            try:
...
'''
if text.count(old) != 1:
    raise SystemExit("config_flow validation block marker mismatch")
p.write_text(text.replace(old, new, 1))
...
if text.count(old) != 1:
    raise SystemExit("sensor import marker mismatch")
text = text.replace(old, new, 1)
marker = "PARALLEL_UPDATES = 0\n\n\n"
helper = '''PARALLEL_UPDATES = 0
...
'''
if text.count(marker) != 1:
    raise SystemExit("sensor helper marker mismatch")
text = text.replace(marker, helper, 1)
old = '''    coordinator: Lotto645Coordinator = entry.runtime_data
    entities: list[SensorEntity] = [
'''
new = '''    coordinator: Lotto645Coordinator = entry.runtime_data
    _prune_stale_optional_sensor_entities(hass, entry, coordinator)
    entities: list[SensorEntity] = [
'''
if text.count(old) != 1:
    raise SystemExit("sensor setup marker mismatch")
p.write_text(text.replace(old, new, 1))

for path, korean in (
    ("custom_components/lotto_645/strings.json", False),
    ("custom_components/lotto_645/translations/ko.json", True),
):
    p = Path(path)
    data = json.loads(p.read_text())
    saju = data["options"]["step"]["saju"]
    if korean:
        saju["description"] = (
            "명리 권장 번호를 사용하기 전에 개인 사주정보를 저장하세요. 출생지는 "
            "대한민국 17개 시·도에서 선택하며 시간대는 Asia/Seoul로 자동 적용합니다. "
            "생년월일은 YYYY-MM-DD, 출생시간은 HH:MM 형식입니다. 원본 출생정보는 외부로 전송하지 않습니다."
        )
        saju["data"]["saju_birth_place"] = "출생지 (대한민국 시·도)"
        saju["data"].pop("saju_timezone", None)
        saju["data_description"]["saju_birth_place"] = (
            "대한민국 17개 시·도에서 선택합니다. 외부 지오코딩/다운로드를 호출하지 않습니다. "
            "기존 버전에서 저장한 해외 출생지는 기존 값만 유지할 수 있습니다."
        )
        saju["data_description"].pop("saju_timezone", None)
        data["options"]["step"]["recommendations"]["data_description"]["selected_methods"] += (
            " 선택에서 해제한 공식의 추천 센서는 재로드 후 엔티티 레지스트리에서 삭제됩니다."
        )
        data["options"]["error"]["select_korean_birthplace"] = (
            "출생지는 대한민국 17개 시·도 중 하나를 선택하세요."
        )
    else:
        saju["description"] = (
            "Save personal birth information before using the Four Pillars recommendation. "
            "Birth place is selected from the 17 first-level South Korean regions and Asia/Seoul is applied automatically. "
            "Raw birth data remains local to Home Assistant."
        )
        saju["data"]["saju_birth_place"] = "Birth place (South Korean region)"
        saju["data"].pop("saju_timezone", None)
        saju["data_description"]["saju_birth_place"] = (
            "Choose one of the 17 South Korean first-level regions. No geocoding or external dataset is requested. "
            "A legacy overseas value already saved by an older release remains selectable only as that saved value."
        )
        saju["data_description"].pop("saju_timezone", None)
        data["options"]["step"]["recommendations"]["data_description"]["selected_methods"] += (
            " Sensors for formulas removed from the selection are deleted from the entity registry after reload."
        )
        data["options"]["error"]["select_korean_birthplace"] = (
            "Choose one of the 17 South Korean regions."
        )
    p.write_text(json.dumps(data, ensure_ascii=False, indent=2) + "\n")

Path("tests/test_korean_birthplaces.py").write_text(
...
p = Path(path)
text = p.read_text()
old = '''        serialize_form(await flow.async_step_recommendations())
        serialize_form(await flow.async_step_saju())
'''
new = '''        serialize_form(await flow.async_step_recommendations())
...
'''
if text.count(old) != 1:
    raise SystemExit("smoke saju form marker mismatch")
text = text.replace(old, new, 1)
old = '''            'saju_birth_time':'1430','saju_gender':'male','saju_birth_place':'SYNTHETIC',
            'saju_timezone':'Asia/Seoul','saju_lunar_leap_month':False, 'saju_true_solar_time':False,
'''
new = '''            'saju_birth_time':'1430','saju_gender':'male','saju_birth_place':'서울특별시',
            'saju_lunar_leap_month':False, 'saju_true_solar_time':False,
'''
if text.count(old) != 1:
    raise SystemExit("smoke saju submit marker mismatch")
text = text.replace(old, new, 1)
old = '''        assert result['data']['saju_birth_time']=='14:30'
        saved=flow_module.Lotto645OptionsFlow(types.SimpleNamespace(options=result['data']))
'''
new = '''        assert result['data']['saju_birth_time']=='14:30'
...
'''
if text.count(old) != 1:
    raise SystemExit("smoke saved marker mismatch")
text = text.replace(old, new, 1)
old = '''        sensor_module=importlib.import_module('custom_components.lotto_645.sensor')
        from homeassistant.helpers.storage import Store
'''
new = '''        sensor_module=importlib.import_module('custom_components.lotto_645.sensor')
...
'''
if text.count(old) != 1:
    raise SystemExit("smoke sensor module marker mismatch")
text = text.replace(old, new, 1)
old = "print('PASS: real HA options menu/forms/JSON serialization/profile save/compact normalization/gating; coordinator manual/AI contracts; purchased five-line round storage, restore, atomic save and draw-name checks')"
new = "print('PASS: real HA options menu/forms; Korea-only birthplace dropdown + implicit Asia/Seoul; deselected formula sensor registry pruning; profile save/gating; coordinator manual/AI contracts; purchased ticket storage and draw checks')"
if text.count(old) != 1:
    raise SystemExit("smoke PASS marker mismatch")
p.write_text(text.replace(old, new, 1))

p = Path("CHANGELOG.md")
text = p.read_text()
section = '''## 1.19.0 — 2026-09-15

- 명리 사주정보의 출생지를 자유 입력 대신 대한민국 17개 시·도 셀렉트박스로 변경했습니다. 새 한국 출생지는 시간대를 `Asia/Seoul`로 자동 저장하며 외부 지오코딩이나 지역 데이터 다운로드가 없습니다. 기존 해외 출생지 값은 호환을 위해 기존 값으로만 유지할 수 있습니다.
- 추첨 공식 선택에서 해제한 공식의 추천 센서는 다음 옵션 재로드 때 엔티티 레지스트리에서도 삭제합니다. AI를 끄면 AI 추천 센서, 명리 공식을 해제하면 명리 프로필 센서도 동일하게 정리하며 추천/리뷰/검증 이력 자체는 보존합니다.
- 실제 Home Assistant selector 직렬화와 오래된 센서 정리 동작을 회귀 테스트에 추가했습니다.

'''
if not text.startswith("# Changelog\n\n"):
    raise SystemExit("CHANGELOG heading mismatch")
p.write_text(text.replace("# Changelog\n\n", "# Changelog\n\n" + section, 1))
Path("CHANGELOG_v1.19.0.md").write_text(
...
Path(".github/workflows/apply-korean-birthplace-prune.yml").unlink(missing_ok=True)
Path("scripts/apply_v1_19_patch.py").unlink()
