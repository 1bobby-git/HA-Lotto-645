"""Safe-area and two-tier header regression on the real panel."""
...
async def verify_safe_area(page: Page) -> None:
    """Exercise inherited HA insets, the two header rows, dialog and notices."""
    await page.evaluate("""() => {
    ...
    for dark in (False, True):
        for width, height, top, right, bottom, left, content_left in CASES:
            ...
            }""", {'top':top,'right':right,'bottom':bottom,'left':left,'contentLeft':content_left,'dark':dark})
            result = await page.evaluate("""() => {
                ...
                return {host:box(el),menu:box(el.node('menu')),title:box(s.querySelector('.ha-component-title')),
            ...
            for key in ('menu','title','brand','settings','tabs'):
            ...
            # Home Assistant header: menu and component name share the first row.
            assert abs((result['menu']['y']+result['menu']['height']/2) - (result['title']['y']+result['title']['height']/2)) <= 4, (label,result)
            assert result['menu']['right'] <= result['title']['x']+1, (label,result)
            assert result['menu']['y'] >= result['host']['y']+top-1, (label,result)
            # Component header is a distinct second row, then tabs.
            assert result['brand']['y'] >= result['menu']['bottom']-1, (label,result)
            assert result['settings']['y'] >= result['menu']['bottom']-1, (label,result)
    ...
    print(f'PASS: two-tier header + safe-area layout {len(CASES)*2} theme/viewport/inset fixtures')
