"""Safe-area layout regression on the real panel, with synthetic inset values.

Called from smoke_ticket_panel.py after its real-module/PNG QR checks. These are
browser layout fixtures, not measurements from a physical iPhone or native app.
"""
...
async def verify_safe_area(page: Page) -> None:
    """Exercise inherited HA insets, orientation, dialogs and zero-inset desktop."""
    await page.evaluate("""() => {
        if (el.node('editor').open) el.node('editor').close();
        el._editing=false;
        el.removeAttribute('data-editor-open');
        el.showScreen('home');
        ...
        for width, height, top, right, bottom, left, content_left in CASES:
            ...
            await page.evaluate("""p => {
                ...
                ['top','right','bottom','left'].forEach(side =>
                    root.setProperty('--safe-area-inset-'+side,p[side]+'px'));
                root.removeProperty('--safe-area-content-inset-left');
                root.removeProperty('--safe-area-content-inset-right');
                if(p.contentLeft!==null)root.setProperty('--safe-area-content-inset-left',p.contentLeft+'px');
                el.hass={...el._hass,themes:{darkMode:p.dark}};
                el.scrollTop=0;
            }""", {'top':top,'right':right,'bottom':bottom,'left':left,
                    'contentLeft':content_left,'dark':dark})
            result = await page.evaluate("""() => {
                ...
                return {host:box(el),menu:box(el.node('menu')),brand:box(el.node('brand-home')),
                  settings:box(s.querySelector('.settings')),tabs:box(s.querySelector('.main-tabs')),
                  top:parseFloat(getComputedStyle(s.querySelector('.app-header')).paddingTop),
            ...
            assert result['top'] == top, (label, result)
            assert not result['overflow'], (label, result)
            for key in ('menu','brand','settings'):
                assert result[key]['y'] >= result['host']['y']+top-1, (label,key,result)
                assert result[key]['right'] <= result['host']['right']-right+1, (label,key,result)
                assert result[key]['x'] >= result['host']['x']+(left if content_left is None else content_left)-1, (label,key,result)
            assert result['menu']['right'] <= result['brand']['x']+1, (label,result)
            assert result['brand']['right'] <= result['settings']['x']+1, (label,result)
            assert result['pageBottom'] >= bottom, (label,result)
            if content_left == 0:
                assert result['left'] == 28, (label,result)  # no double inset on sidebar side
            # Multiple integration selection must stay on its own mobile row.
            await page.evaluate("""() => {
            ...
            dialog = await page.locator('#editor').evaluate("""n=>({
              top:parseFloat(getComputedStyle(n).paddingTop),
              left:parseFloat(getComputedStyle(n).paddingLeft),
              right:parseFloat(getComputedStyle(n).paddingRight),
    ...
    # Environment fallback: remove inherited HA variables, do not manufacture insets.
    await page.evaluate("""() => {
        const root=document.documentElement.style;
        ['top','right','bottom','left'].forEach(side=>{
            root.removeProperty('--safe-area-inset-'+side);
            root.removeProperty('--safe-area-content-inset-'+side);
        });
        el.hass={...el._hass,themes:{darkMode:false}};el.scrollTop=0;
    }""")
    await page.set_viewport_size({'width':1440,'height':1000})
    assert await page.locator('.app-header').evaluate("n=>getComputedStyle(n).paddingTop==='0px'")
    print(f'PASS: safe-area layout {len(CASES)*2} theme/viewport/inset fixtures, header controls, multi-entry row, dialog, notice, zero-inset fallback')
